...
function Library:AddDraggableButton(Text: string, Func)
    local Table = {
        Toggled = false,
        ToggleMode = false, -- 设置为true来启用开关模式
    }

    -- 创建开关容器（模仿UI内部的Toggle样式）
    local Switch = New("Frame", {
        BackgroundColor3 = "MainColor",
        Position = UDim2.fromOffset(6, 6),
        Size = UDim2.fromOffset(54, 24), -- 与UI内部开关统一大小
        ZIndex = 10,
        Parent = ScreenGui,
    ...
    New("UICorner", {
        CornerRadius = UDim.new(0.5, 0), -- 0.5创建椭圆形
        Parent = Switch,
    })
    Library:MakeOutline(Switch, Library.CornerRadius, 9)
    ...
    local Ball = New("Frame", {
        ...
        Size = UDim2.fromScale(0.8, 0.8), -- 滑块相对大小
        SizeConstraint = Enum.SizeConstraint.RelativeYY,
        Parent = Switch,
    })
    New("UICorner", {
        CornerRadius = UDim.new(0.5, 0), -- 滑块也做成椭圆形
        Parent = Ball,
    })
    ...
    -- 点击事件
    Switch.InputBegan:Connect(function(Input: InputObject)
    ...
    Library:MakeDraggable(Switch, Switch, true)
    ...
    function Table:Display()
        ...
        -- 更新开关状态显示
        local offset = Table.Toggled and 0.6 or 0.1
        TweenService:Create(Ball, Library.TweenInfo, {
            Position = UDim2.fromScale(offset, 0.5),
        }):Play()
        
        Switch.BackgroundColor3 = Table.Toggled and Library.Scheme.AccentColor or Library.Scheme.MainColor
        Library.Registry[Switch].BackgroundColor3 = Table.Toggled and "AccentColor" or "MainColor"
    end
    ...
    function Table:SetText(NewText: string)
        ...
        Label.Size = UDim2.fromOffset(X + 6, 20)
        Switch.Size = UDim2.fromOffset(X + 54, 24) -- 自动调整宽度
    end
