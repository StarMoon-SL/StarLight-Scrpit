...
local isStudio, LocalPlayer = RunService:IsStudio(), Players.LocalPlayer
...
local function GetGui()
	...
	local parent = isStudio and LocalPlayer:FindFirstChild("PlayerGui") or (gethui and gethui()) or (cloneref and cloneref(MacLib.GetService("CoreGui")) or MacLib.GetService("CoreGui"))
	gui.Parent = parent
	return gui
end
...
function MacLib:Window(Settings)
	...
	local base = CreateInstance("Frame", {
		...
		Position = UDim2.fromScale(0.5,0.5), Size = Settings.Size or UDim2.fromOffset(868,650)
	}, macLib)
	...
	CreateInstance("UIPadding", {PaddingLeft = UDim.new(0,11)}, controls)
	...
	local controlBtn = function(name, color, layout)
		local btn = CreateInstance("TextButton", {
			Name = name, AutoButtonColor = false, BackgroundColor3 = color, BorderSizePixel = 0, LayoutOrder = layout or 0,
			Text = "", Events = {MouseButton1Click = function() if name == "Exit" then macLib:Destroy() elseif name == "Minimize" then base.Visible = false end end}
		}, controls)
		CreateInstance("UICorner", {CornerRadius = UDim.new(1,0)}, btn)
		return btn
	end
	
	local exit = controlBtn("Exit", Color3.fromRGB(250,93,86))
	local minimize = controlBtn("Minimize", Color3.fromRGB(252,190,57), 1)
	controlBtn("Maximize", Color3.fromRGB(119,174,94), 1).Size = UDim2.fromOffset(7,7)
	
	if Settings.DisabledWindowControls then
		for _, name in ipairs(Settings.DisabledWindowControls) do
			local btn = name == "Exit" and exit or minimize
			btn.Size, btn.Active, btn.Interactable, btn.BackgroundTransparency = UDim2.fromOffset(7,7), false, false, 1
			CreateInstance("UIStroke", {ApplyStrokeMode = Enum.ApplyStrokeMode.Border, Color = Color3.new(1,1,1), Transparency = 0.9}, btn)
		end
	end
	...
	local userId, headshotImage = LocalPlayer.UserId, Players:GetUserThumbnailAsync(LocalPlayer.UserId, Enum.ThumbnailType.AvatarBust, Enum.ThumbnailSize.Size48x48)
	...
	CreateInstance("TextLabel", {
		Name = "DisplayName", FontFace = Font.new(assets.interFont, Enum.FontWeight.SemiBold), Text = LocalPlayer.DisplayName,
	...
	CreateInstance("TextLabel", {
		Name = "Username", FontFace = Font.new(assets.interFont, Enum.FontWeight.SemiBold), Text = "@"..LocalPlayer.Name,
		TextColor3 = Color3.new(1,1,1), TextSize = 12, TextTransparency = 0.7, TextTruncate = Enum.TextTruncate.SplitWord,
	...
	CreateInstance("ImageButton", {
		...
		Visible = dragStyle == 1, Events = {
			InputBegan = function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then dragging = true; dragStart = input.Position; startPos = base.Position end end,
			InputChanged = function(input) if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then dragInput = input end end,
			InputEnded = function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then dragging = false end end
		}
	}, topElements)
	
	CreateInstance("TextLabel", {
		Name = "CurrentTab", FontFace = Font.new(assets.interFont), Text = "", TextTransparency = 0.5,
		TextXAlignment = Enum.TextXAlignment.Left, AnchorPoint = Vector2.new(0,0.5),
		AutomaticSize = Enum.AutomaticSize.Y, BackgroundTransparency = 1, Position = UDim2.fromScale(0,0.5), Size = UDim2.fromScale(0.9,0)
	}, topElements)
	...
	if acrylicBlur then
		...
		local function UpdateOrientation()
			...
			for _, pt in pairs({camera:ScreenPointToRay(tl.x, tl.y).Origin, camera:ScreenPointToRay(tr.x, tr.y).Origin, 
				camera:ScreenPointToRay(bl.x, bl.y).Origin, camera:ScreenPointToRay(br.x, br.y).Origin}) do
		...
		function SectionFuncs:Tab(TabData)
			...
			function TabFuncs:Section(SectionData)
				...
				function SecFuncs:Slider(SliderData, Flag)
					...
					head.InputBegan:Connect(function(inp) if inp.UserInputType == Enum.UserInputType.MouseButton1 then dragging = true; SetValue(inp) end end)
					head.InputEnded:Connect(function(inp) if inp.UserInputType == Enum.UserInputType.MouseButton1 then dragging = false; if SliderData.onInputComplete then SliderData.onInputComplete(finalValue) end end end)
