...
MacLib.MobileConfig = {
    ...
    DefaultMobileSize = UDim2.fromOffset(400, 280),
    MinMobileSize = Vector2.new(350, 250),
...
MacLib.MobileConfig.Enabled = CheckMobile()
...
function MacLib:Window(Settings)
    ...
    base.Size = MacLib.MobileConfig.Enabled and (Settings.Size or MacLib.MobileConfig.DefaultMobileSize) or (Settings.Size or MacLib.MobileConfig.DefaultPCSize)
    ...
    if MacLib.MobileConfig.Enabled then
        local ToggleIcon = assets.menuIcon
        local T = Instance.new("ImageButton", macLib)
        T.Name = "MobileFloatingButton"
        local sizeValue = 56
        T.Size = UDim2.new(0, sizeValue, 0, sizeValue)
        T.Position = UDim2.new(1, -70, 1, -120)
        T.Image = ToggleIcon
        T.BackgroundColor3 = Color3.fromRGB(87, 86, 86)
        T.BorderSizePixel = 0
        T.ZIndex = 100
        ...
        local function updateButtonColor()
            Stroke.Color = baseUIStroke.Color
        end
        ...
        T.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
                ...
                holdTween.Completed:Connect(function()
                    if isHolding and not didLongPress then
                        didLongPress = true
        ...
        T.MouseEnter:Connect(function()
            TweenService:Create(T, TweenInfo.new(0.2), {Size = UDim2.new(0, sizeValue + 6, 0, sizeValue + 10)}):Play()
        end)

        T.MouseLeave:Connect(function()
            TweenService:Create(T, TweenInfo.new(0.2), {Size = UDim2.new(0, sizeValue, 0, sizeValue)}):Play()
        end)
        ...
        MacLib.MobileConfig.FloatingButton = T
        baseUIStroke:GetPropertyChangedSignal("Color"):Connect(updateButtonColor)
        updateButtonColor()
    end
    ...
    if MacLib.MobileConfig.Enabled then
        ...
        sidebar.Size = UDim2.fromOffset(MacLib.MobileConfig.SidebarCompactWidth, sidebar.AbsoluteSize.Y)
        MacLib.MobileConfig.SidebarExpanded = false

        SidebarToggle.MouseButton1Click:Connect(function()
            MacLib.MobileConfig.SidebarExpanded = not MacLib.MobileConfig.SidebarExpanded
            local targetWidth = MacLib.MobileConfig.SidebarExpanded and MacLib.MobileConfig.SidebarExpandedWidth or MacLib.MobileConfig.SidebarCompactWidth
            ...
            TweenService:Create(SidebarToggle, TweenInfo.new(0.2), {Rotation = MacLib.MobileConfig.SidebarExpanded and 0 or 180}):Play()
        end)
