...
local TweenService = StarLight.GetService("TweenService")
local RunService = StarLight.GetService("RunService")
local HttpService = StarLight.GetService("HttpService")
local ContentProvider = StarLight.GetService("ContentProvider")
local UserInputService = StarLight.GetService("UserInputService")
local Lighting = StarLight.GetService("Lighting")
local Players = StarLight.GetService("Players")
...
local function GetGui()
	...
	local parent = RunService:IsStudio() 
		...
		or (cloneref and cloneref(StarLight.GetService("CoreGui")) or StarLight.GetService("CoreGui"))
...
function StarLight:Window(Settings)
		...
		function SectionFunctions:Tab(Settings)
			...
			function TabFunctions:Section(Settings)
				...
				function SectionFunctions:Button(Settings, Flag)
					...
					if Flag then
						StarLight.Options[Flag] = ButtonFunctions
					end
				...
				function SectionFunctions:Toggle(Settings, Flag)
					...
					if Flag then
						StarLight.Options[Flag] = ToggleFunctions
					end
				...
				function SectionFunctions:Slider(Settings, Flag)
					...
					if Flag then
						StarLight.Options[Flag] = SliderFunctions
					end
				...
				function SectionFunctions:Input(Settings, Flag)
					...
					if Flag then
						StarLight.Options[Flag] = InputFunctions
					end
				...
				function SectionFunctions:Keybind(Settings, Flag)
					...
					if Flag then
						StarLight.Options[Flag] = KeybindFunctions
					end
				...
				function SectionFunctions:Dropdown(Settings, Flag)
					...
					if Flag then
						StarLight.Options[Flag] = DropdownFunctions
					end
				...
				function SectionFunctions:Colorpicker(Settings, Flag)
					...
					if Flag then
						StarLight.Options[Flag] = ColorpickerFunctions
					end
				...
				function SectionFunctions:Header(Settings, Flag)
					...
					if Flag then
						StarLight.Options[Flag] = HeaderFunctions
					end
				...
				function SectionFunctions:Label(Settings, Flag)
					...
					if Flag then
						StarLight.Options[Flag] = LabelFunctions
					end
				...
				function SectionFunctions:SubLabel(Settings, Flag)
					...
					if Flag then
						StarLight.Options[Flag] = SubLabelFunctions
					end
				...
				function SectionFunctions:Paragraph(Settings, Flag)
					...
					if Flag then
						StarLight.Options[Flag] = ParagraphFunctions
					end
			...
			function TabFunctions:InsertConfigSection(Side)
				...
				local configSelection = configSection:Dropdown({
					...
					Multi = false,
					Required = false,
					Options = StarLight:RefreshConfigList(),
				...
				configSection:Button({
					...
					Callback = function()
						...
						local success, returned = StarLight:SaveConfig(inputPath)
						if not success then
							WindowFunctions:Notify({
								Title = "Interface",
						...
						configSelection:InsertOptions(StarLight:RefreshConfigList())
					end,
				})
				...
				configSection:Button({
					Name = "Load Config",
					Callback = function()
						local success, returned = StarLight:LoadConfig(configSelection.Value)
						if not success then
							WindowFunctions:Notify({
								Title = "Interface",
				...
				configSection:Button({
					...
					Callback = function()
						local success, returned = StarLight:SaveConfig(configSelection.Value)
						if not success then
							WindowFunctions:Notify({
								Title = "Interface",
				...
				configSection:Button({
					...
					Callback = function()
						...
						writefile(StarLight.Folder .. "/settings/autoload.txt", name)
				...
				if isfile(StarLight.Folder .. "/settings/autoload.txt") then
					local name = readfile(StarLight.Folder .. "/settings/autoload.txt")
					autoloadLabel:UpdateName("Autoload config: " .. name)
				end
			end
	...
	local ClassParser = {
		["Toggle"] = {
			...
			Load = function(Flag, data)
				if StarLight.Options[Flag] and data.state then
					StarLight.Options[Flag]:UpdateState(data.state)
				end
			end
		},
		["Slider"] = {
			...
			Load = function(Flag, data)
				if StarLight.Options[Flag] and data.value then
					StarLight.Options[Flag]:UpdateValue(data.value)
				end
			end
		},
		["Input"] = {
			...
			Load = function(Flag, data)
				if StarLight.Options[Flag] and data.text and type(data.text) == "string" then
					StarLight.Options[Flag]:UpdateText(data.text)
				end
			end
		},
		["Keybind"] = {
			...
			Load = function(Flag, data)
				if StarLight.Options[Flag] and data.bind then
					StarLight.Options[Flag]:Bind(Enum.KeyCode[data.bind])
				end
			end
		},
		["Dropdown"] = {
			...
			Load = function(Flag, data)
				if StarLight.Options[Flag] and data.value then
					StarLight.Options[Flag]:UpdateSelection(data.value)
				end
			end
		},
		["Colorpicker"] = {
			...
			Load = function(Flag, data)
				...
				if StarLight.Options[Flag] and data.color then
					StarLight.Options[Flag]:SetColor(HexToColor3(data.color)) 
					if data.alpha then
						StarLight.Options[Flag]:SetAlpha(data.alpha)
					end
				end
			end
		}
	...
	function StarLight:LoadAutoLoadConfig()
		...
		if isfile(StarLight.Folder .. "/settings/autoload.txt") then
			...
			local suc, err = StarLight:LoadConfig(name)
			if not suc then
				WindowFunctions:Notify({
					Title = "Interface",
	...
	function StarLight:SetFolder(Folder)
		if isStudio then return "Config system unavailable." end

		StarLight.Folder = Folder;
		BuildFolderTree()
	end

	function StarLight:SaveConfig(Path)
		...
		local fullPath = StarLight.Folder .. "/settings/" .. Path .. ".json"

		local data = {
			objects = {}
		}

		for flag, option in next, StarLight.Options do
	...
	function StarLight:LoadConfig(Path)
		if isStudio or not (isfile and readfile) then return "Config system unavailable." end
		...
		local file = StarLight.Folder .. "/settings/" .. Path .. ".json"
	...
	function StarLight:RefreshConfigList()
		if isStudio or not (isfolder and listfiles) then return "Config system unavailable." end

		local list = (isfolder(StarLight.Folder) and isfolder(StarLight.Folder .. "/settings")) and listfiles(StarLight.Folder .. "/settings") or {}
...
if RunService:IsStudio() then
    ...
    pcall(function() StarLight.DevicePlatform = UserInputService:GetPlatform() end)
...
StarLight.Locked = false
local function createMobileControls(WindowFunctions)
    local mobileGui = Instance.new("ScreenGui")
    mobileGui.Name = "StarLightMobileControls"
    mobileGui.ResetOnSpawn = false
    mobileGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    mobileGui.Parent = (gethui and gethui()) or (StarLight.GetService("CoreGui"))
    local btnToggle = Instance.new("TextButton")
    btnToggle.Name = "ToggleBtn"
    btnToggle.Text = "Toggle"
    btnToggle.Size = UDim2.fromOffset(72,36)
    btnToggle.Position = UDim2.new(0,12,1,-60)
    btnToggle.AnchorPoint = Vector2.new(0,0)
    btnToggle.Parent = mobileGui
    btnToggle.BackgroundTransparency = 0.4
    btnToggle.BackgroundColor3 = StarLight.Scheme.MainColor or Color3.fromRGB(25,25,25)
    btnToggle.TextColor3 = StarLight.Scheme.FontColor or Color3.new(1,1,1)
    btnToggle.Font = StarLight.Scheme.Font or Enum.Font.Code
    btnToggle.TextSize = 16
    local btnLock = Instance.new("TextButton")
    btnLock.Name = "LockBtn"
    btnLock.Text = StarLight.Locked and "Lock" or "Unlock"
    btnLock.Size = UDim2.fromOffset(72,36)
    btnLock.Position = UDim2.new(0,12,1,-108)
    btnLock.AnchorPoint = Vector2.new(0,0)
    btnLock.Parent = mobileGui
    btnLock.BackgroundTransparency = 0.4
    btnLock.BackgroundColor3 = StarLight.Scheme.MainColor or Color3.fromRGB(25,25,25)
    btnLock.TextColor3 = StarLight.Scheme.FontColor or Color3.new(1,1,1)
    btnLock.Font = StarLight.Scheme.Font or Enum.Font.Code
    btnLock.TextSize = 16
    btnToggle.MouseButton1Click:Connect(function()
        pcall(function()
            if WindowFunctions and WindowFunctions.ToggleMenu then
                WindowFunctions.ToggleMenu()
            else
    ...
    btnLock.MouseButton1Click:Connect(function()
        StarLight.Locked = not StarLight.Locked
        btnLock.Text = StarLight.Locked and "Lock" or "Unlock"
    end)
    return mobileGui, btnToggle, btnLock
end

StarLight.QuickMenu = {Items = {}}
local function createQuickMenu()
    local qGui = Instance.new("ScreenGui")
    qGui.Name = "StarLightQuickMenu"
    qGui.ResetOnSpawn = false
    qGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    qGui.Parent = (gethui and gethui()) or (StarLight.GetService("CoreGui"))
    local frame = Instance.new("Frame")
    frame.Name = "QuickMenuFrame"
    frame.Size = UDim2.fromOffset(220,300)
    frame.Position = UDim2.new(1,-240,0.5,-150)
    frame.AnchorPoint = Vector2.new(0,0)
    frame.Parent = qGui
    frame.BackgroundColor3 = StarLight.Scheme.MainColor or Color3.fromRGB(25,25,25)
    ...
    frame.Visible = false
    StarLight.QuickMenu.Gui = qGui
    StarLight.QuickMenu.Frame = frame
    return StarLight.QuickMenu
end
local function registerQuickMenuItem(name, getStateFunc, onToggleFunc)
    local qm = StarLight.QuickMenu
    if not qm.Gui then qm = createQuickMenu() end
    local frame = qm.Frame
    local btn = Instance.new("TextButton")
    btn.Size = UDim2.fromOffset(200,36)
    btn.BackgroundColor3 = StarLight.Scheme.BackgroundColor or Color3.fromRGB(15,15,15)
    btn.TextColor3 = StarLight.Scheme.FontColor or Color3.new(1,1,1)
    btn.Font = StarLight.Scheme.Font or Enum.Font.Code
    btn.TextSize = 14
    btn.Text = name
    btn.Parent = frame
    btn.MouseButton1Click:Connect(function()
        if onToggleFunc then pcall(onToggleFunc) end
    end)
    qm.Items[#qm.Items+1] = {Name = name, Button = btn, GetState = getStateFunc, Toggle = onToggleFunc}
    return btn
end
StarLight.RegisterQuickMenuItem = registerQuickMenuItem
local function toggleQuickMenu()
    if not StarLight.QuickMenu.Gui then createQuickMenu() end
    local frame = StarLight.QuickMenu.Frame
    frame.Visible = not frame.Visible
end
StarLight.ToggleQuickMenu = toggleQuickMenu

local oldWindow = StarLight.Window
StarLight.Window = function(self, Settings)
    local WindowFunctions = oldWindow(self, Settings)
    if StarLight.IsMobile then
        pcall(function()
            createMobileControls(WindowFunctions)
        end)
    end
    if WindowFunctions and WindowFunctions.ToggleMenu == nil then
        WindowFunctions.ToggleMenu = function()
            StarLight.Toggled = not StarLight.Toggled
        end
    end
    return WindowFunctions
...
local oldRegisterKeybind = StarLight.RegisterKeybind
if oldRegisterKeybind then
    StarLight.RegisterKeybind = function(self, name, key, callback)
        local ok, res = pcall(function() return oldRegisterKeybind(self, name, key, callback) end)
        pcall(function()
            registerQuickMenuItem(name, function() return false end, function() pcall(callback) end)
        end)
